...
class SubjectsAddFragment : Fragment() {
    ...
    private fun insertDataToDB(nameAdd: Editable, levelAdd: Editable, tagAdd: Editable) {
        ...
        if (inputCheck(nameAdd, levelAdd, tagAdd)) {
            ...
            if (tagStat == null) {
                ...
                mTagStatsViewModel.updateLevel(tagStat.uid,
                    ((Integer.parseInt(tagStat.level) + Integer.parseInt(levelAdd.toString()))/2).toString())
            }

            val subject = Subject(0, nameAdd.toString(), Integer.parseInt(levelAdd.toString()), tagAdd.toString())
            mSubjectViewModel.createSubject(subject)
            Toast.makeText(requireContext(), "Successfully created Subject", Toast.LENGTH_LONG).show()
    ...
    private fun inputCheck(nameAdd: Editable, levelAdd: Editable, tagAdd: Editable
    ): Boolean {
